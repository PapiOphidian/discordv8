...
const TokenCacher = require("../Util/TokenCacher");
...
class InternalClient {
				...
				return new Promise((resolve, reject) => {
					...
					let check = data => {
						if (data.t === "VOICE_SERVER_UPDATE") {
							...
							let chan = new VoiceConnection(channel, this.client, session, token, server, endpoint);
	...
	createServer(name, region = "london") {
		...
		return this.apiRequest('post', Constants.Endpoints.SERVERS, true, { name, region }).then(res => {
	...
	joinServer(invite) {
		...
		return this.apiRequest("post", Constants.Endpoints.INVITE(invite), true).then(res => {
	...
	updateServer(server, options) {
		...
		if (options.verificationLevel) {
			options.verification_level = options.verificationLevel;
		}
		...
		return this.apiRequest("patch", Constants.Endpoints.SERVER(server.id), true, options).then(res => {
	...
	leaveServer(srv) {
		...
		return this.apiRequest("del", Constants.Endpoints.ME_SERVER(server.id), true);
	}
	...
	deleteServer(srv) {
		...
		return this.apiRequest("del", Constants.Endpoints.SERVER(server.id), true);
	}
	...
	logout() {
		...
		if (!this.user.bot) {
			return this.apiRequest("post", Constants.Endpoints.LOGOUT, true).then(disconnect);
		} else {
			return disconnect();
		}
	}
	...
	startPM(resUser) {
		...
		// start the PM
		return this.apiRequest("post", Constants.Endpoints.ME_CHANNELS, true, {
	...
	getGateway() {
		...
		// @ts-ignore
		return this.apiRequest("get", Constants.Endpoints.GATEWAY, true).then(res => this.gatewayURL = res.url);
	}
	...
	sendMessage(where, _content, options = {}) {
		...
		return this.resolver.resolveChannel(where).then(destination => {
			...
			if (options.file) {
				return this.resolver.resolveFile(options.file.file).then(file => this.apiRequest("post", Constants.Endpoints.CHANNEL_MESSAGES(destination.id), true, params, {
					name: options.file.name,
					file: file
				}).then(res => destination.messages.add(new Message(res, destination, this.client))));
			} else {
				return this.apiRequest("post", Constants.Endpoints.CHANNEL_MESSAGES(destination.id), true, params).then(res => destination.messages.add(new Message(res, destination, this.client)));
			}
		});
	}
	...
	sendFile(where, _file, name, content) {
		...
		return this.resolver.resolveChannel(where).then(channel => this.resolver.resolveFile(_file).then(file => this.apiRequest("post", Constants.Endpoints.CHANNEL_MESSAGES(channel.id), true, content, {
			name,
			file
	...
	deleteMessage(_message, options = {}) {
		...
		return chain.then(() => this.apiRequest("del", Constants.Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id), true)).then(() => message.channel.messages.remove(message));
	}

	// def deleteMessages
	deleteMessages(_messages) {
		if (!Array.isArray(_messages)) return Promise.reject(new Error("Messages provided must be in an array"));
		...
		return this.apiRequest("post", `${Constants.Endpoints.CHANNEL_MESSAGES(channel.id)}/bulk_delete`, true, {
	...
	updateMessage(msg, _content, options = {}) {
		...
		return this.apiRequest("patch", Constants.Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id), true, params).then(res => message.channel.messages.update(message, new Message(res, message.channel, this.client)));
	}
	...
	getChannelLogs(_channel, limit = 50, options = {}) {
		return this.resolver.resolveChannel(_channel).then(channel => {
			...
			return this.apiRequest("get", `${Constants.Endpoints.CHANNEL_MESSAGES(channel.id)}?${querystring.stringify(qsObject)}`, true).then(res => res.map(msg => channel.messages.add(new Message(msg, channel, this.client))));
		});
	}
	...
	getMessage(_channel, messageID) {
		return this.resolver.resolveChannel(_channel).then(channel => {
			...
			return this.apiRequest("get", `${Constants.Endpoints.CHANNEL_MESSAGES(channel.id)}/${messageID}`, true).then(res => channel.messages.add(new Message(res, channel, this.client)));
		});
	}
	...
	pinMessage(msg) {
		...
		return this.apiRequest("put", `${Constants.Endpoints.CHANNEL_PIN(msg.channel.id, msg.id)}`, true);
	}
	...
	unpinMessage(msg) {
		...
		return this.apiRequest("del", `${Constants.Endpoints.CHANNEL_PIN(msg.channel.id, msg.id)}`, true);
	}

	// def getPinnedMessages
	getPinnedMessages(_channel) {
		return this.resolver.resolveChannel(_channel).then(channel => {
			return this.apiRequest("get", `${Constants.Endpoints.CHANNEL_PINS(channel.id)}`, true).then(res => res.map(msg => channel.messages.add(new Message(msg, channel, this.client))));
		});
	}
	...
	getBans(server) {
		...
		return this.apiRequest("get", Constants.Endpoints.SERVER_BANS(server.id), true).then(res => res.map(ban => this.users.add(new User(ban.user, this.client))));
	}
	...
	createChannel(server, name, type = 0) {
		...
		return this.apiRequest("post", Constants.Endpoints.SERVER_CHANNELS(server.id), true, {
			name,
			type
		}).then(res => {
			let channel;
	...
	deleteChannel(_channel) {
		...
		return this.resolver.resolveChannel(_channel).then(channel => this.apiRequest("del", Constants.Endpoints.CHANNEL(channel.id), true).then(() => {
			if (channel.server) {
	...
	banMember(user, server, length = 1) {
		...
		return this.apiRequest("put", `${Constants.Endpoints.SERVER_BANS(server.id)}/${user.id}?delete-message-days=${length}`, true);
	}
	...
	unbanMember(user, server) {
		...
		return this.apiRequest("del", `${Constants.Endpoints.SERVER_BANS(server.id)}/${user.id}`, true);
	}
	...
	kickMember(user, server) {
		...
		return this.apiRequest("del", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true);
	}
	...
	moveMember(user, channel) {
		...
		return this.resolver.resolveChannel(channel).then(channel => {
			...
			if (channel.type !== 2) {
				...
				return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true, { channel_id: channel.id }).then(res => {
	...
	muteMember(user, server) {
		user = this.resolver.resolveUser(user);
		server = this.resolver.resolveServer(server);
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true, { mute: true });
	}
	...
	unmuteMember(user, server) {
		user = this.resolver.resolveUser(user);
		server = this.resolver.resolveServer(server);
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true, { mute: false });
	}
	...
	deafenMember(user, server) {
		user = this.resolver.resolveUser(user);
		server = this.resolver.resolveServer(server);
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true, { deaf: true });
	}
	...
	undeafenMember(user, server) {
		user = this.resolver.resolveUser(user);
		server = this.resolver.resolveServer(server);
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id}`, true, { deaf: false });
	}
	...
	setNickname(server, nick, user) {
		...
		user = this.resolver.resolveUser(user);
		server = this.resolver.resolveServer(server);
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(server.id)}/${user.id === this.user.id ? "@me/nick" : user.id}`, true, { nick: nick });
	}
	...
	setNote(user, note) {
		...
		return this.apiRequest("put", `${Constants.Endpoints.ME_NOTES}/${user.id}`, true, { note: note });
	}
	...
	createRole(server, data) {
		...
		return this.apiRequest("post", Constants.Endpoints.SERVER_ROLES(server.id), true).then(res => {
	...
	updateRole(role, data) {
		...
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_ROLES(server.id)}/${role.id}`, true, newData).then(res => {
	...
	deleteRole(role) {
		if (role.server.id === role.id) {
			...
			return this.apiRequest("del", `${Constants.Endpoints.SERVER_ROLES(role.server.id)}/${role.id}`, true);
		}
	}
	...
	addMemberToRole(member, roles) {
		...
		return this.apiRequest("patch", `${Constants.Endpoints.SERVER_MEMBERS(roles[0].server.id)}/${member.id}`, true, {
			roles: roleIDs
		});
	}
	...
	createInvite(chanServ, options) {
		return this.resolver.resolveChannel(chanServ).then(channel => {
			...
			return this.apiRequest("post", Constants.Endpoints.CHANNEL_INVITES(channel.id), true, options).then(res => new Invite(res, this.channels.get("id", res.channel.id), this.client));
		});
	}
	...
	deleteInvite(invite) {
		...
		return this.apiRequest("del", Constants.Endpoints.INVITE(invite), true);
	}
	...
	getInvite(invite) {
		...
		return this.apiRequest("get", Constants.Endpoints.INVITE(invite), true).then(res => {
			if (!this.channels.has("id", res.channel.id)) {
				return new Invite(res, null, this.client);
			}
			return this.apiRequest("post", Constants.Endpoints.CHANNEL_INVITES(res.channel.id), true, { validate: invite }).then(res2 => new Invite(res2, this.channels.get("id", res.channel.id), this.client));
		});
	}
	...
	getInvites(channel) {
		if (!(channel instanceof Channel)) {
			...
			if (server) {
				return this.apiRequest("get", Constants.Endpoints.SERVER_INVITES(server.id), true).then(res => {
					return res.map(data => new Invite(data, this.channels.get("id", data.channel.id), this.client));
				});
			}
		}
		return this.resolver.resolveChannel(channel).then(channel => {
			return this.apiRequest("get", Constants.Endpoints.CHANNEL_INVITES(channel.id), true).then(res => {
				return res.map(data => new Invite(data, this.channels.get("id", data.channel.id), this.client));
			});
		});
	}
	...
	overwritePermissions(channel, role, updated) {
		return this.resolver.resolveChannel(channel).then(channel => {
			...
			return this.apiRequest("put", `${Constants.Endpoints.CHANNEL_PERMISSIONS(channel.id)}/${data.id}`, true, data);
		});
	}
	...
	//def sendTyping
	sendTyping(channel) {
		return this.resolver.resolveChannel(channel).then(channel => this.apiRequest("post", Constants.Endpoints.CHANNEL(channel.id) + "/typing", true));
	}
	...
	updateDetails(data) {
		...
		return this.apiRequest("patch", Constants.Endpoints.ME, true, options);
	}
	...
	updateChannel(channel, data) {
		return this.resolver.resolveChannel(channel).then(channel => {
			...
			return this.apiRequest("patch", Constants.Endpoints.CHANNEL(channel.id), true, data).then(res => {
	...
	addFriend(user) {
		...
		let id;
		if (user instanceof String || typeof user === "string") id = user;else if (user instanceof User) {
			user = this.resolver.resolveUser(user);
			id = user.id;
		} else {
			if (user.username && user.discriminator) // add by username and discriminator (pass in an object)
				return this.apiRequest("put", Constants.Endpoints.FRIENDS, true, user);else return Promise.reject("invalid user");
		}

		return this.apiRequest("put", `${Constants.Endpoints.FRIENDS}/${id}`, true, {});
	}
	...
	removeFriend(user) {
		...
		return this.apiRequest("delete", `${Constants.Endpoints.FRIENDS}/${user.id}`, true);
	}
	...
	getServerWebhooks(server) {
		...
		return this.apiRequest("get", Constants.Endpoints.SERVER_WEBHOOKS(server.id), true).then(res => res.map(webhook => {
			let channel = this.channels.get("id", webhook.channel_id);
			return channel.webhooks.add(new Webhook(webhook, server, channel, this.users.get("id", webhook.user.id)));
		}));
	}
	...
	getChannelWebhooks(channel) {
		return this.resolver.resolveChannel(channel).then(channel => {
			...
			return this.apiRequest("get", Constants.Endpoints.CHANNEL_WEBHOOKS(channel.id), true).then(res => res.map(webhook => channel.webhooks.add(new Webhook(webhook, this.servers.get("id", webhook.guild_id), channel, this.users.get("id", webhook.user.id)))));
		});
	}
	...
	editWebhook(webhook, options = {}) {
		return this.resolver.resolveWebhook(webhook).then(webhook => {
			...
			return this.apiRequest("patch", Constants.Endpoints.WEBHOOK(webhook.id), true, options).then(res => {
	...
	createWebhook(channel, options = {}) {
		return this.resolver.resolveChannel(channel).then(destination => {
			...
			return this.apiRequest("post", Constants.Endpoints.CHANNEL_WEBHOOKS(destination.id), true, options).then(webhook => channel.webhooks.add(new Webhook(webhook, this.servers.get("id", webhook.guild_id), channel, this.users.get("id", webhook.user.id))));
		});
	}
	...
	deleteWebhook(webhook) {
		return this.resolver.resolveWebhook(webhook).then(webhook => {
			...
			return this.apiRequest("delete", Constants.Endpoints.WEBHOOK(webhook.id), true).then(() => {
	...
	sendWebhookMessage(webhook, _content, options = {}) {
		return this.resolver.resolveWebhook(webhook).then(destination => {
			...
			return this.apiRequest("post", `${Constants.Endpoints.WEBHOOK_MESSAGE(destination.id, destination.token)}${slack ? "/slack" : ""}?wait=true`, true, options);
		});
	}
	...
	getOAuthApplication(appID) {
		appID = appID || "@me";
		return this.apiRequest("get", Constants.Endpoints.OAUTH2_APPLICATION(appID), true);
	}
	...
	ack(msg) {
		...
		return this.apiRequest("post", Constants.Endpoints.CHANNEL_MESSAGE(msg.channel.id, msg.id) + "/ack", true);
	}
	...
	createWS(url) {
		...
		this._inflate = new Unpacker.default(this.erlpack, this.zlib, this.receiveDecompressedPacket.bind(this));
	...
	processPacket(packet) {
		...
		let data = packet.d;
		switch (packet.t) {
			case Constants.PacketType.RESUMED:
			...
			case Constants.PacketType.MESSAGE_CREATE:
				{
			...
			case Constants.PacketType.MESSAGE_DELETE:
				{
					let channel = this.channels.get("id", data.channel_id) || this.private_channels.get("id", data.channel_id);
					if (channel) {
						// potentially blank
			...
			case Constants.PacketType.MESSAGE_DELETE_BULK:
				{
					let channel = this.channels.get("id", data.channel_id) || this.private_channels.get("id", data.channel_id);
					if (channel) {
						data.ids.forEach(id => {
			...
			case Constants.PacketType.MESSAGE_UPDATE:
				{
					// format https://discordapi.readthedocs.org/en/latest/reference/channels/messages.html#message-format
					let channel = this.channels.get("id", data.channel_id) || this.private_channels.get("id", data.channel_id);
					if (channel) {
			...
			case Constants.PacketType.SERVER_CREATE:
				{
			...
			case Constants.PacketType.SERVER_DELETE:
				{
					let server = this.servers.get("id", data.id);
					if (server) {
						if (!data.unavailable) {
			...
			case Constants.PacketType.SERVER_UPDATE:
				{
					let server = this.servers.get("id", data.id);
					if (server) {
						// server exists
			...
			case Constants.PacketType.CHANNEL_CREATE:
				{
			...
			case Constants.PacketType.CHANNEL_DELETE:
				{
			...
			case Constants.PacketType.CHANNEL_UPDATE:
				{
					let channel = this.channels.get("id", data.id) || this.private_channels.get("id", data.id);
					if (channel) {
						...
						if (channel instanceof PMChannel) {
							//PM CHANNEL
							client.emit("channelUpdated", new PMChannel(channel, client), this.private_channels.update(channel, new PMChannel(data, client)));
						} else {
							if (channel.server) {
								if (channel.type === 0) {
									//TEXT CHANNEL
			...
			case Constants.PacketType.SERVER_ROLE_CREATE:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
			...
			case Constants.PacketType.SERVER_ROLE_DELETE:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
			...
			case Constants.PacketType.SERVER_ROLE_UPDATE:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
			...
			case Constants.PacketType.SERVER_MEMBER_ADD:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
						...
						client.emit("serverNewMember", server, server.members.add(this.users.add(new User(data.user, client))));
					} else {
						client.emit("warn", "server member added but server doesn't exist in cache");
					}
					break;
				}
			case Constants.PacketType.SERVER_MEMBER_REMOVE:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
			...
			case Constants.PacketType.SERVER_MEMBER_UPDATE:
				{
					let server = this.servers.get("id", data.guild_id);
					if (server) {
			...
			case Constants.PacketType.PRESENCE_UPDATE:
				{
			...
			case Constants.PacketType.USER_UPDATE:
				{
			...
			case Constants.PacketType.TYPING:
				{
			...
			case Constants.PacketType.SERVER_BAN_ADD:
				{
					let user = this.users.get("id", data.user.id);
					let server = this.servers.get("id", data.guild_id);
			...
			case Constants.PacketType.SERVER_BAN_REMOVE:
				{
					let user = this.users.get("id", data.user.id);
					let server = this.servers.get("id", data.guild_id);
			...
			case Constants.PacketType.USER_NOTE_UPDATE:
				{
					if (this.user.bot) {
						return;
					}
			...
			case Constants.PacketType.VOICE_STATE_UPDATE:
				{
					let user = this.users.get("id", data.user_id);
					let server = this.servers.get("id", data.guild_id);
			...
			case Constants.PacketType.SERVER_MEMBERS_CHUNK:
				{
			...
			case Constants.PacketType.FRIEND_ADD:
				{
					if (this.user.bot) {
						return;
					}
			...
			case Constants.PacketType.FRIEND_REMOVE:
				{
					if (this.user.bot) {
						return;
					}
			...
			case Constants.PacketType.SERVER_SYNC:
				{
