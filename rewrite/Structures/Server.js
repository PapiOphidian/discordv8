...
const strictKeys = ["region", "ownerID", "name", "id", "icon", "afkTimeout", "afkChannelID"];
...
class Server extends Equality {
	...
	constructor(data, client) {
		...
		if (data.presences) {
			for (const presence of data.presences) {
				const user = client.internal.users.get("id", presence.user.id);
				if (user) {
	...
	toObject() {
		const keys = ['id', 'name', 'region', 'ownerID', 'icon', 'afkTimeout', 'afkChannelID', 'large', 'memberCount'],
		    obj = {};
	...
	detailsOf(user) {
		...
		if (user) {
			const result = this.memberMap[user.id] || {};
	...
	get iconURL() {
		if (!this.icon) {
			...
			return Constants.Endpoints.SERVER_ICON(this.id, this.icon);
		}
	}
	...
	eventVoiceJoin(user, channel) {
		...
		const oldChannel = this.eventVoiceLeave(user);
	...
	leave() {
		return this.client.leaveServer.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}
	...
	createInvite() {
		return this.client.createInvite.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}

	createRole() {
		return this.client.createRole.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}
	...
	getBans() {
		return this.client.getBans.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}

	createChannel() {
		return this.client.createChannel.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}

	setNickname() {
		return this.client.setNickname.apply(this.client, (ArgumentRegulariser.reg)(this, arguments));
	}
