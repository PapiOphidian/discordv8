...
class AudioEncoder {
	...
	sanityCheck() {
		const _opus = this.opus;
		const encodeZeroes = function () {
			try {
				const zeroes = new Buffer(1920);
				zeroes.fill(0);
	...
	getCommand(force) {
		if (this.choice && force) return this.choice;

		const choices = ["avconv", "ffmpeg"];

		for (const choice of choices) {
			const p = child_process.spawnSync(choice);
			if (!p.error) {
				// @ts-ignore
				this.choice = choice;
				return choice;
	...
	encodeStream(stream, options) {
		return new Promise((resolve, reject) => {
			...
			// @ts-ignore
			const enc = child_process.spawn(command, ['-i', '-', '-f', 's16le', '-ar', '48000', '-ss', options.seek || 0, '-ac', 2, 'pipe:1']);

			const dest = stream.pipe(enc.stdin);
	...
	encodeFile(file, options) {
		return new Promise((resolve, reject) => {
			...
			// @ts-ignore
			const enc = child_process.spawn(command, ['-i', file, '-f', 's16le', '-ar', '48000', '-ss', options.seek || 0, '-ac', 2, 'pipe:1']);
	...
	encodeArbitraryFFmpeg(ffmpegOptions, options) {
		return new Promise((resolve, reject) => {
			...
			// @ts-ignore
			const enc = child_process.spawn(command, ffmpegOptions);
