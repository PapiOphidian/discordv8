...
const VoicePacket = require("./VoicePacket");
const VolumeTransformer = require("./VolumeTransformer");
const StreamIntent = require("./StreamIntent");
...
class VoiceConnection extends events.EventEmitter {
	...
	playStream(stream, channels = 2) {
		...
		function send() {
			...
			try {
				...
				let buffer = stream.read(1920 * channels);
				...
				if (buffer.length !== 1920 * channels) {
					const newBuffer = new Buffer(1920 * channels).fill(0);
				...
				const nextTime = startTime + count * length;
	...
	setSpeaking(value) {
		this.playing = value;
		if (this.vWS.readyState === ws.OPEN) this.vWS.send(JSON.stringify({
			op: 5,
			d: {
				speaking: value,
				delay: 0
	...
	sendPacket(packet, callback = function (err) {}) {
		...
		self.playing = true;
		try {
			if (self.vWS.readyState === ws.OPEN) self.udp.send(packet, 0, packet.length, self.vWSData.port, self.endpoint, callback);
		} catch (e) {
			self.playing = false;
			callback(e);
			return false;
	...
	sendBuffer(rawbuffer, sequence, timestamp, callback) {
		...
		try {
			...
			const buffer = self.encoder.opusBuffer(rawbuffer);
			const packet = new VoicePacket(buffer, sequence, timestamp, self.vWSData.ssrc, self.secret);
	...
	playFile(stream, options = false, callback = function (err, str) {}) {
		...
		return new Promise((resolve, reject) => {
			this.encoder.encodeFile(stream, options).then(data => {
				...
				const intent = self.playStream(data.stream, 2);
				resolve(intent);
	...
	playRawStream(stream, options = false, callback = function (err, str) {}) {
		...
		return new Promise((resolve, reject) => {
			this.encoder.encodeStream(stream, options).then(data => {
				...
				const intent = self.playStream(data.stream);
				resolve(intent);
				callback(null, intent);
			}).catch(error);
			function error(e = true) {
	...
	init() {
		...
		dns.lookup(this.endpoint, (err, address, family) => {
			const vWS = self.vWS = new ws("wss://" + this.endpoint, null, { rejectUnauthorized: false });
			this.endpoint = address;
			const udpClient = self.udp = dgram.createSocket("udp4");
			...
			udpClient.on('message', function (msg, rinfo) {
				const buffArr = JSON.parse(JSON.stringify(msg)).data;
				if (firstPacket === true) {
					for (let i = 4; i < buffArr.indexOf(0, i); i++) {
						discordIP += String.fromCharCode(buffArr[i]);
					}
					discordPort = msg.readUIntLE(msg.length - 2, 2).toString(10);

					const modes = self.vWSData.modes;
					let mode = MODE_xsalsa20_poly1305;
				...
				switch (data.op) {
					case 2:
						...
						self.KAI = KAI = self.client.internal.intervals.misc["voiceKAI"] = setInterval(() => {
							if (vWS && vWS.readyState === ws.OPEN) vWS.send(JSON.stringify({
								op: 3,
								d: null
							}));
						}, data.d.heartbeat_interval);

						const udpPacket = new Buffer(70);
					...
					case 4:
						...
						break;
					case 5:
						const user = self.server.members.get("id", data.d.user_id);

						if (user) {
							const speaking = data.d.speaking;
